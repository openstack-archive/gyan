...
"""Gyan common internal object model"""

from oslo_versionedobjects import base as ovoo_base
from oslo_versionedobjects import fields as ovoo_fields

from gyan.objects import fields as obj_fields

remotable_classmethod = ovoo_base.remotable_classmethod
remotable = ovoo_base.remotable


class GyanObjectRegistry(ovoo_base.VersionedObjectRegistry):
    pass


class GyanObject(ovoo_base.VersionedObject):
    ...
    as appropriate.
    """
    OBJ_SERIAL_NAMESPACE = 'gyan_object'
    OBJ_PROJECT_NAMESPACE = 'gyan'
...
class GyanPersistentObject(object):
...
class GyanObjectSerializer(ovoo_base.VersionedObjectSerializer):
    # Base class to use for object hydration
    OBJ_BASE_CLASS = GyanObject
...
def obj_to_primitive(obj):
    ...
    A GyanObject becomes a dict, and anything that implements ObjectListBase
    becomes a list.
    """
    if isinstance(obj, ObjectListBase):
        return [obj_to_primitive(x) for x in obj]
    elif isinstance(obj, GyanObject):
        result = {}
...
def obj_equal_prims(obj_1, obj_2, ignore=None):
    ...
    if ignore is not None:
        keys = ['gyan_object.changes'] + ignore
    else:
        keys = ['gyan_object.changes']
